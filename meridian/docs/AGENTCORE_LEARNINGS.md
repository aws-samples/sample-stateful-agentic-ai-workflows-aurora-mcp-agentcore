...
- **App Runner in us-east-1 rejected every optional setting** with the same message: a custom auto scaling configuration, a health check at a 10 second interval (HTTP or TCP), an explicit default egress configuration, and any tag list (empty or not) each failed on their own while the identical service without the setting deployed at the same moment. CloudFormation's resource handler always sends a tag list, so no stack-created service ever deployed; `scripts/publish.py` now creates the service with the SDK from a bare definition, waits for RUNNING, and retries a failed creation. The image still comes from the `MeridianWebBackend` stack and the site stack gets the service host.
- **App Runner also refused the Secrets Manager reference for the token**: a service with `RuntimeEnvironmentSecrets` failed while the same service with the value as a plain runtime environment variable ran; the token is injected as an environment variable and the secret stays as the operator's record.
- **App Runner also refused a service whose port stayed closed while the app loaded**: on one vCPU the backend needs close to thirty seconds to import its dependencies and initialise the checkpoint backend, and the deployment failed before an instance was provisioned whenever the port opened that late, while the same image with an instantly open port deployed. `backend/launch.py` binds port 8000 in milliseconds and hands the socket to uvicorn with `--fd`; early connections wait in the kernel backlog until startup completes. A probe that streamed uvicorn's output and timing to CloudWatch from inside the container (`boto3` `put_log_events`, since App Runner ships no application log for a failed deployment) is what showed the startup time.
- **App Runner service creation also had a window of failing everything**, including three identical known-good services, for about 20 minutes after a burst of experiments; a quiet quarter hour cleared it.
