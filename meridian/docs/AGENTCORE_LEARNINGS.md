...
- **App Runner service creation also fails intermittently** with the same message for definitions that deploy fine minutes later, including a control that had just passed. The service is its own stack (`MeridianWebBackend`) so `publish.py` retries it without recreating CloudFront and the bucket.
