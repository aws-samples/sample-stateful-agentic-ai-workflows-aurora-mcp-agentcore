...
4. Verify `StorageEncryptionType` and the requested key mode, private writer, Data API, TLS, database users/RLS/grants, exact catalog/booking/journey counts and checkpoint/lease contents on the replacement. A snapshot also retains database connection/read-only settings: connect to `postgres` on the replacement to allow `meridian` connections, preserving the read-only default during the comparison. Test reads with the existing application secret ARN; do not reveal or rotate its value. Run isolated transaction/rollback checks. No reseeding.
